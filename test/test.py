...
# include files
from testwebsitecrawler.test_local_functions import *
from testwebsitecrawler.test_ftp_functions import *


# Create and configure logger
logging.basicConfig(filename=os.getcwd()+"/testlogs/"+str(date.today())+".log", format='%(asctime)s %(message) s', filemode='w')

# set name of the directory to which websites are going to save as a global variable
savedWebsitesDirName = "/testsavedwebsites/"

# main program
def main():
	localDir = os.getcwd() + savedWebsitesDirName	# set working directory
	
	with open('test_webpages_to_crawl.csv') as csv_file:
